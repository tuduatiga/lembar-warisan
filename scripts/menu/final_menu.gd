...
func _set_final(final_text: String, score: int, score_multiplier: int, room_cleared: int, room_total: int) -> void:
	self.final_label.text = final_text
	self.score_label.text = "Skor: " + str(score) + " (" + str(score_multiplier) + "x)"
	self.room_cleared_label.text = "Ruang Dimurnikan: " + str(room_cleared) + " / " + str(room_total)
	self.final_score_label.text = "Skor Akhir: " + str(round(score * (float(room_cleared) / float(room_total)) * 100) / 100)
