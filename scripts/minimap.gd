...
func _draw() -> void:
	...
	for dungeon_area in self.get_tree().get_nodes_in_group("DungeonArea"):
		if dungeon_area is DungeonArea:
			if dungeon_area._visited:
				var dungeon_dist_sq: float = player_pos.distance_squared_to(
					dungeon_area.global_position
				)
	...
	var origin: Vector2 = self.size / 2.0 - (nearest_pos / SCALE)

	var first_dungeon_area: Variant = self.get_tree().get_first_node_in_group("DungeonArea")
	if first_dungeon_area:
		draw_line(
			origin,
			origin + first_dungeon_area.global_position / SCALE,
			VISITED_COLOR if first_dungeon_area._visited else UNVISITED_COLOR,
			ROOM_RECT_SIZE / 4
		)
	draw_rect(
		Rect2(
	...
	for dungeon_area in self.get_tree().get_nodes_in_group("DungeonArea"):
		if dungeon_area is DungeonArea:
			...
			var find_dungeon_area_impl: Callable = func(marker: Node) -> Array[Node]:
				return marker.get_child(0).find_children("*", "DungeonArea", false)

			var find_dungeon_area: Callable = func(gen_marker: Node) -> Variant:
				return gen_marker.find_children("*", "Marker2D").map(find_dungeon_area_impl).reduce(
					func(acc: Array[Node], other_das: Array[Node]) -> Array[Node]:
						return acc + other_das,
					[] as Array[Node]
				)

			var other_dungeon_areas: Array[Node] = dungeon_area.owner.find_children("*", "GenMarker").map(find_dungeon_area).reduce(
				func(acc: Array[Node], other_das: Array[Node]) -> Array[Node]:
					return acc + other_das,
				[] as Array[Node]
			)
			...
			for other_dungeon_area in other_dungeon_areas:
				draw_line(
					origin + dungeon_area.global_position / SCALE,
					origin + other_dungeon_area.global_position / SCALE,
					VISITED_COLOR if other_dungeon_area._visited else UNVISITED_COLOR,
					ROOM_RECT_SIZE / 4
				)
